...
const EmployeeRequestDashboard: React.FC = () => {
    ...
    async function fetchAssignments(): Promise<void> {
        try {
            const response = await axiosPrivate.get('/employee/assignments');
    ...
    async function updateAssignments(assignmentId: number, status: AssignmentStatus, comment: string): Promise<void> {
        try {
            await axiosPrivate.post(`/employee/assignments/id=${assignmentId}`,
                {
                    status: status,
                    comment: comment
                }
                                                    ...
                                                    <tr key={assignment.id}>
                                                        ...
                                                        <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">
                                                            <div className="flex items-center gap-x-2">
                                                                <Avatar size={'sm'} />
                                                                <div>
                                                                    <h2 className="text-sm font-medium text-gray-800 dark:text-white ">{auth.firstname + ' ' + auth.lastname}</h2>
                                                                    <p className="text-xs font-normal text-gray-600 dark:text-gray-400">{auth.email}</p>
                                                                </div>
                                                            </div>
                                                        </td>
